...
def sigmoid_relevance(y, cl, ch):
    ...
    Use sigmoid_relevance if y follows a normal distribution. Otherwise,
    use dist_relevance or a step relevance function.
...
def pdf_relevance(y, bandwidth=1.0):
    ...
    probability density function (PDF). A kernel PDF is evaluated using 
    sklearn.neighbors.KernelDensity, and then normalized.
...
def bin_split(y, bins, verbose=False):
    """ 
    ...
    verbose : boolean, optional (default=False)
        If True, print a dataframe showing the range and the frequency of each bin
    
    Returns
    --------
    bin_indices : list
        A list of indices for each bin
        
    Example
    --------
    >>> y = np.random.uniform(0, 1, size=100)
    >>> bin_indices = bin_split(y, bins=[0.2, 0.5, 0.6, 0.8])
    ...
    if verbose:
        bin_freqs = [len(x) for x in bin_indices]
        bin_freqs += [np.sum(bin_freqs)]
        bin_ranges = ['y < ' + str(bins[0])] + \
                        ['{0} ≤ y < {1}'.format(bins[i], bins[i+1]) \
                         for i in range(len(bins)-1)] + \
                         ['y ≥ ' + str(bins[-1])] + ['TOTAL']
                        
        df = pd.DataFrame([bin_ranges, bin_freqs]).transpose()
        df.columns = ['bin range', 'frequency']
        print('\n')
...
def uniform_test_split(X, y, bins, bin_test_size=0.5, verbose=False, random_state=None):
    ...
    ...                                         bin_test_size=0., max_bin_ratio=0.4)
    ...
    bin_indices = bin_split(y, bins, verbose=False)
...
def accuracy_score(y_true, y_pred, error_threshold, normalize=True):
    ...
    y_true. A prediction is accurate if the absolute error is not more than 
    error_threshold.
...
def precision_score(y_true, y_pred, error_threshold, relevance_pred, relevance_threshold, 
    ...
    ..  [1] Torgo, L., and Ribeiro, R. (2009). Precision and recall for regression.
        International Conference on Discovery Science. Springer, Berlin, Heidelberg, 2009.
...
def bin_performance(y_true, y_pred, bins, metric='MSE', error_threshold=None):
    ...
    assert len(y_true)==len(y_pred), 'y_true and y_pred must be of the same length.'
    bin_indices = bin_split(y_true, bins, verbose=False)
    perf_bins = []
...
def random_oversample(X, y, relevance, relevance_threshold=0.5, over='balance', 
    ...
    ..  [1] Branco, P., Torgo, L., and Ribeiro, R.P. (2019). Pre-processing approaches for
        imbalanced distributions in regression. Neurocomputing 343:76-99.    
    """
...
def smoter(X, y, relevance, relevance_threshold=0.5, k=5, over='balance', under=None, nominal=None,
           random_state=None):
    """
    ...
    under : float (default=None)
        ...
        over is float. Half of normal samples are removed if under=0.5.
    nominal : ndarray (default=None)
        Column indices of nominal features. If None, then all features are continuous.
    random_state : int or None, optional (default=None)
        If int, random_state is the seed used by the random number generator. If None, 
    ...
    References
    -----------
    ..  [1] Torgo, L. et al (2015). Resampling strategies for regression. Expert Systems
        32.3:465-476
...
class Rebagg():
    ...
    References
    -----------
    ..  [1] Branco, P. et al (2018). Rebagg: Resampled bagging for imbalanced regression.
        Second International Workshop on Learning with Imbalanced Domains: Theory and 
        Applications.
    ...
    def sample(self, X, y, relevance, relevance_threshold, 
        ...
        if sample_method in ('random_oversample', 'smoter', 'gaussian'):
            ...
            if s_rare <= len(y_rare_all):
                ...
                if sample_method=='random_oversample':
                    ...
                    if k is None:
                        raise ValueError("k must be specified if sample_method is "\
                                         "smoter")
                    ...
                    if delta is None:
                        raise ValueError("delta must be specified if sample_method is "\
                                         "gaussian")
                    X_rare, y_rare = oversample(X_rare_all, y_rare_all, size=s_rare,
        ...
        elif 'wercs' in sample_method:
            if None in [over, under]:
                raise ValueError("Must specify the value of over and under if "\
                                 "sample_method is 'wercs' or 'wercs-gn'")
            if sample_method=='wercs-gn' and delta is None:
                raise ValueError("Must specify the value of delta if sample_method is "\
                                 "'wercs-gn'")
